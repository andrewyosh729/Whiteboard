...
let existingDrawingsString = document.getElementById("data-container");
let existingDrawings = JSON.parse(existingDrawingsString.getAttribute('existing-drawings'));
let mouseDown = false;
...
existingDrawings.forEach(drawing => {
    console.log(drawing.DrawingPoints);
    let drawingLine = CreateLine();
    drawing.DrawingPoints.forEach(point => {
        console.log(point)
        UpdateDrawing(drawingLine, point.X, point.Y)
    });
    scene.add(drawingLine.line)
...
async function onMouseUpdate(e) {
    ...
    if (mouseDown) {
        if (currentLocalDrawingData == null) {
            currentLocalDrawingData = CreateLine();
...
function UpdateDrawing(drawingData, mouseX, mouseY) {
    drawingData.drawingPoints.push(mouseX, mouseY, 0)
    drawingData.geometry.setPoints(drawingData.drawingPoints);
}